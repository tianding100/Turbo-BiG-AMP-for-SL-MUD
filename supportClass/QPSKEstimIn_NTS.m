classdef QPSKEstimIn_NTS < EstimIn
    ...
    methods
        ...
        function [xhat, xvar, val] = estim(obj, rhat, rvar)
            ...
            con_prob1 = exp( -abs( 1 - rhat).^2 ./ rvar) / pi ./ rvar;
            con_probm1 = exp( -abs( -1 - rhat).^2 ./ rvar) / pi ./ rvar;
            con_probi = exp( -abs( 1i - rhat).^2 ./ rvar) / pi ./ rvar;
            con_probmi = exp( -abs( -1i - rhat).^2 ./ rvar) / pi ./ rvar;
...
%                 % Compute the negative KL divergence
%                 %   klDivNeg = \sum_i \int p(x|r)*\log( p(x) / p(x|r) )dx
%                 klDivNeg1 = con_prob1 .* log( prior_prob1 ./ con_prob1);
%                 klDivNegm1 = con_probm1 .* log( prior_probm1 ./ con_probm1);
%                 klDivNegi = con_probi .* log( prior_probi ./ con_probi);
%                 klDivNegmi = con_probmi .* log( prior_probmi ./ con_probmi);
...
%           logpy = - log(pi) - log(rvar) + log(py1 + pym1 + pymi + pyi);
