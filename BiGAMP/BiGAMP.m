...
%Initial block Inference options
if opt.blockInference
    optInference.L = L;
    optInference.N = N;
    optInference.B = B;
    optInference.spar = problem.spar;
    optInference.p = problem.p;
    optInference.maxlogRatio = 50;
end
...
while ~stop
    ...
    if compVal
        ...
        %Block sparsity model inference
        if (opt.blockInference) && (mod(it,100) == 99)
            optInference.logRatio = gX.computeLogRatio(rhat, rvar);
            newSparsity = inferenceBlock(optInference);
            gX.p1 = newSparsity;
            estFin.nzLocation = inferenceBlockHard(optInference);
        end
        [xhat,xvar,valInX] = gX.estim(rhat, rvar*pvar_mean);        %anchor
